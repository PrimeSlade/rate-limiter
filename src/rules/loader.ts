...
export function loadRules(configPath?: string): RulesConfig {
  ...
  for (const [domain, rule] of Object.entries(raw.domains)) {
    if (!rule.algorithm) throw new Error(`Domain "${domain}": missing algorithm`)
    if (!['token_bucket'].includes(rule.algorithm)) {
      throw new Error(`Domain "${domain}": unknown algorithm "${rule.algorithm}"`)
    }
    ...
    if (!Number.isFinite(rule.refill_rate) || rule.refill_rate <= 0) {
      throw new Error(`Domain "${domain}": refill_rate must be a positive number`)
    }
    if (!rule.identifier) throw new Error(`Domain "${domain}": missing identifier`)
    ...
    if (refill_rate_per_ms <= 0) {
      throw new Error(`Domain "${domain}": computed refill_rate_per_ms must be > 0`)
    }
