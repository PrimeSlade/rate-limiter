...
async function fetchOverride(domain: string): Promise<Partial<DomainRule> | null> {
  const now = Date.now()
  ...
  overrideCache.set(domain, { rule: override, expiresAt: now + OVERRIDE_TTL_MS })
  return override
}
...
export function extractIdentifier(rule: DomainRule, req: IncomingMessage): string {
